...
class LLMDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # 2. 提取字段 (使用 .get() 防止字段缺失报错)
        instruction = line.get('instruction', '')
        input_val = line.get('input', '')
        output = line.get('output', '')

        # 3. 拼接文本
        # 策略：将 instruction 和 input 作为输入部分，output 作为回答部分
        # 这种格式有助于模型学习“指令->回答”的逻辑

        if input_val:
            # 如果有 input 内容（例如翻译任务）
            prompt = f"### Instruction:\n{instruction}\n\n### Input:\n{input_val}\n\n### Response:\n{output}"
        else:
            # 如果没有 input 内容（例如写诗任务）
            prompt = f"### Instruction:\n{instruction}\n\n### Response:\n{output}"

        # 加上开始和结束符 (根据你的模型调整，例如 LLaMA 使用 <s> 和 </s>)
        text = '<s>' + prompt + '</s>'
